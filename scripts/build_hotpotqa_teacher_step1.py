...
def load_rollout_step1(rollout_path: Path) -> List[dict]:
    records = []
    seen_qids = set()
    ...
    for row_idx, record in enumerate(read_jsonl(rollout_path), start=1):
        ...
        t = int(record["t"])
        if t != 1:
            raise ValueError(f"当前脚本只处理 step1，但发现 t={t}: qid={qid}")
...
def build_teacher_split(rollout_path: Path, targets_path: Path, teacher_path: Path) -> Dict[str, int]:
    rollout_records = load_rollout_step1(rollout_path)
...
def main():
    ...
    print("HotpotQA teacher step1 构建完成：")
    for split in ["train", "val", "test"]:
