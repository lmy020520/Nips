...
def convert_split(
    *,
    ...
    stop_probe = load_stop_probe(stop_probe_path)
    ...
    def generator():
        for qid in sorted(queries.keys()):
            ...
            if qid not in gates:
                raise ValueError(f"gates 中找不到 qid: {qid}")
            if qid not in stop_probe:
                raise ValueError(f"stop_probe 中找不到 qid: {qid}")

            yield build_record_for_qid(
                qid=qid,
                queries=queries,
...
def main():
    ...
    for split in SPLITS:
        ...
        gates_path = trajectories_dir / f"gates_{split}.jsonl"
        stop_probe_path = trajectories_dir / f"stop_probe_{split}.jsonl"
        raw_units_path = unit_registry_dir / raw_units_name_map[split]
        output_path = trajectories_dir / out_name_map[split]

        for path in [queries_path, init_state_path, candidates_path, gates_path, stop_probe_path, raw_units_path]:
            if not path.exists():
        ...
        stats[split] = convert_split(
            ...
            stop_probe_path=stop_probe_path,
    ...
    print("derived_harvest v2 构建完成：")
    print(f"  defaults: J={TOP_RAW_J}, max_candidates={MAX_CANDIDATES}")
    for split in SPLITS:
