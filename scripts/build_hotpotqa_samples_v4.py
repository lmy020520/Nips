...
DEFAULT_SAMPLES_BASE = os.environ.get("HOTPOTQA_SAMPLES_BASE", os.environ.get("HOTPOTQA_DATA_ROOT", "data/hotpotqa_distractor_v4") + "/samples")
TOP_RAW_J = 3
...
def build_aux_candidate_provenance(
    *,
    g_t_aux: List[str],
    g_t_illegal: List[str],
    provenance_index: Dict[str, Dict[str, Dict[str, Optional[str]]]],
) -> Dict[str, Dict[str, Optional[str]]]:
    out: Dict[str, Dict[str, Optional[str]]] = {}
    for status, unit_ids in [("aux", g_t_aux), ("illegal", g_t_illegal)]:
        for unit_id in unit_ids:
            if unit_id in out:
                continue
            payload = build_raw_provenance(unit_id, provenance_index)
            payload["provenance_scope"] = "aux"
...
def build_candidates_block(
    ...
    *,
    ...
    return {
        ...
        "aux_candidate_provenance": build_aux_candidate_provenance(
            g_t_aux=g_t_aux,
            g_t_illegal=g_t_illegal,
            provenance_index=provenance_index,
        ),
...
def build_labels_block(
    ...
    *,
    ...
    candidate_set = set(
        str(x)
        for x in expect_list(candidates_block.get("C_t", []), name="candidates_block.C_t")
    )
    negative_unit_ids = [
        str(x)
        for x in expect_list(ranking_rec["negative_unit_ids"], name="ranking_rec.negative_unit_ids")
        if str(x) in candidate_set and str(x) != positive_unit_id
    ]
...
def build_sample_for_key(
    ...
    *,
    ...
    return {
        ...
        "build_meta": {
            "run_id": run_id,
            "source": "build_hotpotqa_samples_v4.py",
            "split": split,
        },
...
def validate_top_level(record: dict) -> Tuple[str, int]:
    ...
    if source != "build_hotpotqa_samples_v4.py":
...
def validate_candidates_block(
    ...
    required = ["R_t", "G_t_final", "G_t_aux", "G_t_illegal", "C_t", "candidate_provenance", "aux_candidate_provenance"]
    for key in required:
    ...
    aux_candidate_provenance = expect_dict(candidates["aux_candidate_provenance"], name="candidates.aux_candidate_provenance")

    expected_c_t = merge_unique_in_order(r_t + g_t_final)
    if c_t != expected_c_t:
        raise ValueError("candidates.C_t 必须严格等于 R_t ∪ G_t_final（保持顺序去重后）")

    if set(candidate_provenance.keys()) != set(c_t):
        raise ValueError("candidates.candidate_provenance 必须只覆盖 C_t")
    ...
    expected_aux_ids = set(merge_unique_in_order(g_t_aux + g_t_illegal))
    if set(aux_candidate_provenance.keys()) != expected_aux_ids:
        raise ValueError("candidates.aux_candidate_provenance 必须覆盖 G_t_aux/G_t_illegal")
    for unit_id, provenance in aux_candidate_provenance.items():
        provenance = expect_dict(provenance, name=f"candidates.aux_candidate_provenance[{unit_id}]")
        if provenance.get("provenance_scope") != "aux":
            raise ValueError("aux_candidate_provenance.provenance_scope 必须是 aux")
        if provenance.get("candidate_status") not in {"aux", "illegal"}:
            raise ValueError("aux_candidate_provenance.candidate_status 必须是 aux/illegal")
...
def validate_meta_block(meta: dict) -> None:
    ...
    if keep_prefix is not True:
        raise ValueError("最终 samples_v3 中只允许出现 keep_prefix=true 的样本")
...
def validate_record(
    ...
    if forbidden_hits:
        raise ValueError(f"最终 samples_v3 不能包含 debug 字段: {forbidden_hits[:5]}")
...
def main():
    ...
    write_json(
        ...
        samples_base / "build_manifest_v4.json",
    )
