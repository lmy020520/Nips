...
def build_candidates_split(
    ...
    ranking_records = load_ranking_records(ranking_path)
    ...
    def generator():
        for key in sorted(states.keys(), key=lambda x: (x[0], x[1])):
            ...
            used_unit_ids = {item["unit_id"] for item in state["H_t"]}
            ...
            r_t = topk_retrieve(
                ...
                shortlist_k=CHUNK_SHORTLIST_K,
                final_kr=FINAL_KR,
            )

            ranking_rec = ranking_records.get(key)
            positive_unit_id = None
            negative_unit_ids: List[str] = []
            if ranking_rec is not None:
                positive_unit_id = ranking_rec["positive_unit_id"]
                negative_unit_ids = ranking_rec["negative_unit_ids"]

            raw_extras = []
            derived_final = []
            if positive_unit_id is not None:
                if is_derived_unit_id(positive_unit_id):
                    derived_final.append(positive_unit_id)
                else:
                    raw_extras.append(positive_unit_id)

            for unit_id in negative_unit_ids:
                if is_derived_unit_id(unit_id):
                    derived_final.append(unit_id)
                else:
                    raw_extras.append(unit_id)

            r_t = merge_unique_in_order(r_t + raw_extras)
            g_t_final = merge_unique_in_order(derived_final)
            c_t = merge_unique_in_order(r_t + g_t_final)
...
def main():
    ...
    for split in SPLITS:
        ...
        ranking_path = base_dir / "labels" / f"ranking_{split}.jsonl"
        output_path = trajectories_dir / output_name_map[split]

        for path in [queries_path, states_path, chunks_path, atoms_path, raw_units_path, ranking_path]:
            if not path.exists():
    ...
    print("candidates v2 构建完成：")
    print(f"  defaults: chunk_shortlist={CHUNK_SHORTLIST_K}, K_r={FINAL_KR}")
    for split in SPLITS:
