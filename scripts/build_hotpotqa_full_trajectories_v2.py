...
def make_run_id() -> str:
    return time.strftime("%Y%m%d_%H%M%S_hotpotqa_v2")
...
def propose_derived(
    *,
    ...
    if use_v2_prompt:
        user_prompt = build_propose_prompt_v2(
            question,
            state_summary,
            top_raw_candidates,
            gold_answer,
...
def final_retain_selection(
    ...
    *,
    ...
    if derive_mode == "repair_after_false_stop":
...
def rollout_one_qid(
    ...
    for t in range(T_MAX):
        ...
        stop_info_v2 = cheap_stop_gate_v2(
            target_info,
            state["A_t"],
            state["S_t"],
            r_t,
        ...
        derived_info = need_derived_gate_v2(
            query_info["question"],
            target_info,
            state["A_t"],
            state["S_t"],
        ...
        if USE_GATE_V2_FOR_CONTROL:
            stop_info = stop_info_v2
            stop_candidate = bool(stop_info_v2["stop_candidate"])
            need_derived_control = bool(derived_info["trigger_derived"] or repair_override_shadow)
            control_mode = "v2"
            ...
            need_derived_control = bool(derived_info_control["trigger_derived"])
            control_mode = "legacy"
            active_repair_override = False
            active_derive_mode = "normal"
            use_v2_prompt = derive_goal != "generic_bridge_or_verification"
        ...
        gate_trace = {
            "raw_complete": bool(stop_info_v2["raw_complete"]),
            "no_obvious_derived_need": bool(stop_info_v2["no_obvious_derived_need"]),
            "no_recent_closure_failure": bool(stop_info_v2["no_recent_closure_failure"]),
            "stop_candidate": bool(stop_info_v2["stop_candidate"]),
            ...
            "shadow_only": bool(USE_GATE_V2_FOR_DEBUG and not USE_GATE_V2_FOR_CONTROL),
        ...
        log(
            ...
            f"shadow_stop_candidate={stop_info_v2['stop_candidate']} "
        ...
        if triggered_propose_derived or need_derived:
            ...
            if need_derived or active_repair_override:
                ...
                log(
                    ...
                    f"derive_mode={effective_derive_mode} derive_goal={derive_goal}"
                )
        ...
        step_record = {
            ...
            "stop_candidate": stop_candidate,
            "need_derived": bool(need_derived),
            "triggered_propose_derived": bool(triggered_propose_derived),
...
def main():
    ...
    build_source = "build_hotpotqa_full_trajectories_v2.py"

    log("build_hotpotqa_full_trajectories_v2.py started", force=True)
    log(
    ...
    write_json(
        ...
        trajectories_dir / "build_manifest_v2.json",
    )
