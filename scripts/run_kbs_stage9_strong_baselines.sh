...
prepare_answer_caches() {
  if [[ "${KBS_STAGE9_BASELINE_CACHE_PREP_AUTHORIZED:-0}" != "1" ]]; then
    echo "[ERROR] Stage 9.3 answer-cache preparation is locked pending selection review" >&2
    exit 1
  fi
  ...
  echo "FINISHED_OK"
  echo "status=STAGE9_3_ANSWER_CACHE_READINESS_OK"
