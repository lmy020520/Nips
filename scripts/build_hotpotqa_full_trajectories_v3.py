...
def make_run_id() -> str:
    return time.strftime("%Y%m%d_%H%M%S_hotpotqa_v3")
...
def rollout_one_qid(
    ...
    for t in range(T_MAX):
        ...
        if USE_GATE_V2_FOR_CONTROL:
            ...
            # the candidate pool.
            if need_derived_control and derived_subtype_shadow in {"late_verification", "early_bridge"}:
                active_derive_subtype = derived_subtype_shadow
            else:
                active_derive_subtype = None
            use_v2_prompt = bool(
                active_derive_subtype in {"late_verification", "early_bridge"}
...
def main():
    ...
    build_source = "build_hotpotqa_full_trajectories_v3.py"

    log("build_hotpotqa_full_trajectories_v3.py started", force=True)
    log(
    ...
    write_json(
        ...
        trajectories_dir / "build_manifest_v3.json",
    )
