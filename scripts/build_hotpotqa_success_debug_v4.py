...
def write_debug_manifest(base_dir: Path, *, debug_type: str, run_ids_by_split: Dict[str, str]) -> None:
    manifest_path = base_dir / "debug" / "build_manifest_v4.json"
    ...
    manifest["source"] = "build_hotpotqa_success_debug_v4.py" if debug_type == "success" else "build_hotpotqa_failure_debug_v4.py"
...
def build_history_entry(item: dict, unit_registry: Dict[str, dict], derived_payloads: Dict[str, Dict[str, Any]]) -> dict:
...
def build_selected_derived_units(
    *,
    history: List[dict],
    derived_payloads: Dict[str, Dict[str, Any]],
    unit_registry: Dict[str, dict],
    rendered_k_t: str,
    selected_t_by_unit_id: Dict[str, int],
    ...
    for item in history:
        ...
        if not is_derived_unit_id(unit_id):
            continue
        payload = derived_payloads.get(unit_id)
        selected_h_step_id = item.get("step_id")
        selected_t = selected_t_by_unit_id.get(str(unit_id))
        if selected_t is None:
            warnings.append(
                {
                    "type": "missing_selected_t_for_derived",
                    "unit_id": unit_id,
                }
            )
        if not isinstance(payload, dict):
            warnings.append(
                {
                    "type": "missing_selected_derived_payload",
                    "unit_id": unit_id,
                }
            )
            selected_units.append(
                {
                    ...
                    "source_unit_ids": [],
                    "source_unit_texts": [],
                    "selected_h_step_id": selected_h_step_id,
        ...
        normalized_source_unit_ids = [str(x) for x in source_unit_ids] if isinstance(source_unit_ids, list) else []
        source_unit_texts = []
        for source_unit_id in normalized_source_unit_ids:
            raw_source = unit_registry.get(source_unit_id)
            source_text = None if raw_source is None else raw_source.get("text")
            if isinstance(source_text, str) and source_text.strip():
                source_unit_texts.append({"unit_id": source_unit_id, "text": source_text})
            else:
                warnings.append(
                    {
                        "type": "missing_source_unit_text_for_selected_derived",
        ...
        selected_units.append(
            {
                "unit_id": unit_id,
                "type": None if note_type is None else str(note_type),
                "text": text or None,
                "source_unit_ids": normalized_source_unit_ids,
                "source_unit_texts": source_unit_texts,
                "selected_h_step_id": selected_h_step_id,
                "selected_t": selected_t,
                "in_final_K_t": bool(text and text in rendered_k_t),
            }
        )
    return selected_units, warnings


def build_selected_t_by_unit_id(full_rec: dict) -> Dict[str, int]:
    out: Dict[str, int] = {}
    for step in full_rec.get("steps", []):
        if not isinstance(step, dict):
            continue
        unit_id = step.get("positive_unit_id")
        t = step.get("t")
        if is_derived_unit_id(unit_id) and isinstance(t, int) and unit_id not in out:
            out[str(unit_id)] = t
    return out


def validate_history_derived_unit_texts(
    history: List[dict],
    selected_derived_units: List[dict],
) -> List[dict]:
    warnings: List[dict] = []
    selected_text_by_unit = {
        str(item.get("unit_id")): str(item.get("text", "")).strip()
    ...
    for item in history:
        ...
        selected_text = selected_text_by_unit.get(str(unit_id), "")
        if not isinstance(unit_text, str) or not unit_text.strip():
            warnings.append(
                {
                    "type": "missing_h_t_derived_unit_text",
                    "unit_id": unit_id,
                }
            )
            continue
        if selected_text and unit_text.strip() != selected_text:
            warnings.append(
                {
                    "type": "mismatched_h_t_derived_unit_text",
                    "unit_id": unit_id,
                }
            )
    return warnings
...
def build_debug_record(
    *,
    ...
    history = [build_history_entry(item, unit_registry, derived_payloads) for item in terminal_state.get("H_t", [])]
    k_t = render_k_t_with_derived_payloads(str(terminal_state.get("K_t", "")), derived_payloads)
    selected_derived_units, selected_derived_warnings = build_selected_derived_units(
        history=history,
        derived_payloads=derived_payloads,
        unit_registry=unit_registry,
        rendered_k_t=k_t,
        selected_t_by_unit_id=build_selected_t_by_unit_id(full_rec),
    )
    ...
    warnings.extend(selected_derived_warnings)
    warnings.extend(validate_history_derived_unit_texts(history, selected_derived_units))
    ...
    return {
        ...
        "build_meta": {
            "run_id": run_id,
            "source": "build_hotpotqa_success_debug_v4.py",
        },
        ...
        "chunks": chunks,
        "selected_derived_units": selected_derived_units,
...
def build_split(base_dir: Path, split: str) -> Tuple[List[dict], dict, str]:
    ...
    for qid, full_rec in full_by_qid.items():
        ...
        if record["debug_warnings"]:
            stats["warnings"] += 1
            for warning in record["debug_warnings"]:
                if isinstance(warning, str):
                    stats["warning_counts"][warning] += 1
                elif isinstance(warning, dict):
                    stats["warning_counts"][str(warning.get("type", "unknown_warning"))] += 1
