...
ALLOWED_STOP_LABEL_TYPES = {"continue", "near-terminal", "stop"}
BRIDGE_SUBTYPES = {
    "early_bridge",
    "bridge_scaffold",
...
def validate_file_presence(base: Path, issues: List[dict]) -> None:
    required = []
    for split in SPLITS:
        required.extend(
            [
                base / "trajectories" / f"full_{split}.jsonl",
                base / "samples" / f"{split}.jsonl",
                base / "debug" / f"success_semantic_debug_{split}.jsonl",
            ]
        )
    for path in required:
        if not path.exists():
            add_issue(issues, "CRITICAL", split="all", issue="missing_required_file", extra={"detail": str(path)})
...
def load_target_chunk_ids(base: Path) -> Dict[str, set]:
    out: Dict[str, set] = {}
    for split in SPLITS:
        path = base / "targets" / f"{split}.jsonl"
        if not path.exists():
            continue
        for row in read_jsonl(path):
            qid = str(row.get("qid", ""))
            ...
            for item in row.get("T_q_raw") or []:
                if not isinstance(item, dict):
                    continue
                chunk_id = item.get("chunk_id") or item.get("parent_chunk_id") or item.get("unit_id")
                if chunk_id:
...
def raw_parent_chunk_id(unit_id: str) -> str:
    ...
    for split in SPLITS:
        ...
        if full_qids != debug_qids:
            add_issue(
                ...
                "CRITICAL",
                split=split,
                issue="full_debug_qid_set_mismatch",
                extra={"detail": f"missing_debug={sorted(full_qids - debug_qids)[:5]} extra_debug={sorted(debug_qids - full_qids)[:5]}"},
            )
...
def question_terms(question: str) -> List[str]:
    stop = {
        ...
        "both", "type", "types", "name", "called", "made", "by", "as", "with", "from", "at",
        "it", "this", "these", "those", "has", "have", "had", "current", "former",
    }
    return [x for x in re.findall(r"[A-Za-z][A-Za-z]+", normalize_lexical(question)) if x not in stop]


def normalize_lexical(text: Any) -> str:
...
def answer_visible(answer: str, text: str) -> bool:
    ans = normalize_lexical(answer)
    blob = normalize_lexical(text)
    if not ans:
        return False
    if ans in blob:
        return True
    ans_tokens = [tok for tok in ans.split() if len(tok) > 1]
    if len(ans_tokens) >= 2 and all(tok in blob.split() for tok in ans_tokens):
        return True
    return False
...
def relevance_score(question: str, answer: str, text: str) -> int:
    ...
    score = sum(1 for term in terms if term in blob_tokens or (len(term) >= 4 and term in blob))
    if answer_visible(answer, text):
        score += 3
    return score
    ...
    *,
    ...
    for split in SPLITS:
        ...
        for qid, full_row in full_by_split.get(split, {}).items():
            terminal_t_raw = full_row.get("terminal_t")
            if isinstance(terminal_t_raw, int) and not isinstance(terminal_t_raw, bool):
                terminal_t = terminal_t_raw
            else:
                add_issue(issues, "CRITICAL", split=split, qid=qid, issue="full_terminal_t_invalid")
                terminal_t = len(full_row.get("steps") or [])
            ...
            for row in sample_rows:
                ...
                extra_provenance = set(provenance.keys()) - set(c_t)
                if extra_provenance:
                    stats["candidate_provenance_extra_keys"] += len(extra_provenance)
                ...
                stats["aux_candidate_provenance_keys"] += len(aux_prov)
                for aux_unit_id, aux_item in aux_prov.items():
                    if not isinstance(aux_item, dict):
                        add_issue(issues, "CRITICAL", split=split, qid=qid, t=t, issue="aux_candidate_provenance_item_invalid", extra={"detail": aux_unit_id})
                        continue
                    if not aux_item.get("provenance_scope") or not aux_item.get("candidate_status"):
                        add_issue(issues, "CRITICAL", split=split, qid=qid, t=t, issue="aux_candidate_provenance_missing_scope_or_status", extra={"detail": aux_unit_id})
                ...
                if stop_label.get("label_type") == "near-terminal":
                    stats["near_terminal_count"] += 1
                    ...
                    if not (t < terminal_t):
                        add_issue(issues, "CRITICAL", split=split, qid=qid, t=t, issue="near_terminal_t_not_before_terminal")
    ...
    *,
    ...
    for split in SPLITS:
        ...
        for qid, full_row in full_by_split.get(split, {}).items():
            ...
            stats[f"answer_match_rule::{answer_probe.get('answer_match_rule')}"] += 1
            stats[f"support_rule::{answer_probe.get('support_rule')}"] += 1
            if warnings:
                severity = "INFO" if answer_probe.get("AnswerCorrect_t") and answer_probe.get("SupportSufficient_t") else "WARNING"
                add_issue(issues, severity, split=split, qid=qid, issue="debug_warnings_non_empty", extra={"warnings": warnings})
            ...
            snapshot = ((full_row.get("terminal_probe") or {}).get("state_snapshot") or {})
            snapshot_units = sorted_h_units(snapshot)
            debug_units = [
                str(item.get("unit_id"))
                for item in (terminal_state.get("H_t") or [])
                if isinstance(item, dict)
            ]
            if snapshot_units and debug_units != snapshot_units:
                add_issue(issues, "CRITICAL", split=split, qid=qid, issue="debug_selected_path_snapshot_mismatch")
            ...
            if any(is_derived(unit_id) for unit_id in selected_unit_ids):
                ...
                if not selected_derived_units:
                    stats["selected_derived_units_missing"] += 1
            ...
            for unit_id in selected_unit_ids:
                ...
                if item.get("source_unit_ids") and not item.get("source_unit_texts"):
                    stats["selected_derived_source_text_missing"] += 1
                    add_issue(issues, "CRITICAL", split=split, qid=qid, issue="selected_derived_source_text_missing", extra={"unit_id": unit_id})
                text = f"{unit_id} {item.get('text', '')}"
            ...
            raw_units_in_chunks = set()
            for chunk in row.get("chunks", []):
                if not isinstance(chunk, dict):
                    continue
                for unit_id in chunk.get("selected_unit_ids_in_this_chunk") or []:
                    raw_units_in_chunks.add(str(unit_id))
            for unit_id in selected_unit_ids:
                if is_derived(unit_id):
                    continue
                if unit_id not in raw_units_in_chunks:
                    add_issue(issues, "CRITICAL", split=split, qid=qid, issue="selected_raw_missing_from_debug_chunks", extra={"detail": unit_id})
                raw_text = raw_texts.get(unit_id, "")
                is_target_raw = raw_parent_chunk_id(unit_id) in target_chunk_ids.get(qid, set())
                if (not is_target_raw) and raw_text and relevance_score(row.get("question", ""), row.get("gold_answer", ""), raw_text) == 0:
                    relevance_text = f"{unit_id} {raw_text}"
                    if relevance_score(row.get("question", ""), row.get("gold_answer", ""), relevance_text) == 0:
                        add_issue(issues, "WARNING", split=split, qid=qid, issue="selected_raw_low_question_relevance", extra={"detail": unit_id})
            for h_item in terminal_state.get("H_t") or []:
                if not isinstance(h_item, dict) or not is_derived(h_item.get("unit_id")):
                    continue
                stats["h_t_derived_units_total"] += 1
                ...
                if not isinstance(h_text, str) or not h_text.strip():
                    stats["h_t_derived_unit_text_missing"] += 1
                    ...
                    stats["h_t_derived_unit_text_mismatch"] += 1
            ...
            if not question.lower().startswith(("are ", "is ", "was ", "were ")) and answer:
                ...
                if not answer_visible(answer, f"{k_t}\n{chunks_text}"):
...
def print_summary(
    *,
    ...
    answer_match_dist = {
        ...
        if key.startswith("answer_match_rule::")
    }
    support_rule_dist = {
        key.split("::", 1)[1]: value
        for key, value in semantic_stats.items()
        if key.startswith("support_rule::")
    }
    info_items = {
        "near_terminal_count": sample_stats.get("near_terminal_count", 0),
        "terminal_step_distribution": {
            key.removeprefix("path_len_"): value
            for key, value in path_stats.items()
            if key.startswith("path_len_")
        },
        "answer_match_rule_distribution": answer_match_dist,
        "support_rule_distribution": support_rule_dist,
    }
    print("DATASET RELEASE VALIDATION SUMMARY")
    ...
    print("derived:")
    print(f"  derived_positive_total={path_stats.get('derived_positive_total', 0)}")
    print(f"  derived_positive_with_trigger_false={path_stats.get('derived_positive_with_trigger_false', 0)}")
    print(f"  derived_positive_with_subtype_trigger_only_candidate={path_stats.get('trigger_only_positive', 0)}")
    print(f"  derived_positive_type_subtype_mismatch={path_stats.get('type_subtype_mismatch', 0)}")
    print(f"  G_t_final_derived_with_trigger_false={path_stats.get('g_final_derived_with_trigger_false', 0)}")
    print(f"  selected_derived_units_missing={semantic_stats.get('selected_derived_units_missing', 0)}")
    print(f"  selected_derived_source_text_missing={semantic_stats.get('selected_derived_source_text_missing', 0)}")
    print("debug_h_t:")
    print(f"  h_t_derived_units_total={semantic_stats.get('h_t_derived_units_total', 0)}")
    print(f"  h_t_derived_unit_text_missing={semantic_stats.get('h_t_derived_unit_text_missing', 0)}")
    print(f"  h_t_derived_unit_text_mismatch={semantic_stats.get('h_t_derived_unit_text_mismatch', 0)}")
    print("path_quality:")
    ...
    print(f"  terminal_step_distribution={dict(sorted(info_items['terminal_step_distribution'].items(), key=lambda x: int(x[0])))}")
    print("semantic:")
    ...
    print(f"  answer_match_rule_distribution={dict(sorted(answer_match_dist.items()))}")
    print(f"  support_rule_distribution={dict(sorted(support_rule_dist.items()))}")
    print("provenance:")
    print(f"  candidate_provenance_extra_keys={sample_stats.get('candidate_provenance_extra_keys', 0)}")
    print(f"  aux_candidate_provenance_keys={sample_stats.get('aux_candidate_provenance_keys', 0)}")
    print("issues:")
    ...
    for issue in issues:
        ...
        if detail is None:
            detail = " ".join(f"{k}={v}" for k, v in issue.items() if k not in {"severity", "split", "qid", "t", "issue"})
    ...
    if strict and critical == 0 and warning > 0:
        status = "USABLE_WITH_WARNINGS"
    print(f"DATASET STATUS = {status}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Validate HotpotQA trajectory dataset release readiness.")
    parser.add_argument("--data_root", "--base-dir", dest="data_root", type=Path, default=DEFAULT_BASE)
    parser.add_argument("--strict", action="store_true", help="Keep semantic warnings visible in final release status.")
    args = parser.parse_args()

    issues: List[dict] = []
    base = args.data_root
    validate_file_presence(base, issues)
    raw_texts = load_raw_texts(base)
    target_chunk_ids = load_target_chunk_ids(base)
